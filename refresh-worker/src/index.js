...
function normalizeAnalysis(value, context) {
  ...
  const signals = groundedWorkoutSignals(context);
  if (signals.length < 2) {
    signals.push(
  ...
  return {
    ...
    workoutAnalysis: {
      title: groundAerobicEffectText(analysis.workoutAnalysis?.title, context, latest ? `Use ${latestName} to set the cost of the next session` : "Waiting for a completed Garmin workout"),
